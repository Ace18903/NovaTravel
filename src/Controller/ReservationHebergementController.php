...
final class ReservationHebergementController extends AbstractController
{
    ...
    {
        $reservationHebergements = $entityManager
            ->getRepository(ReservationHebergement::class)
            ->findAll();

        return $this->render('reservation_hebergement/index.html.twig', [
            'reservation_hebergements' => $reservationHebergements,
        ]);
    }
    ...
    {
        if ($this->isCsrfTokenValid('delete'.$reservationHebergement->getId(), $request->getPayload()->getString('_token'))) {
